...
export const createRecipeModel = (
	...
	let combinedLayer = tf.layers
		.dense({ units: 64, activation: "relu" })
		.apply(combined);
	combinedLayer = tf.layers
		.dense({ units: 32, activation: "relu" })
		.apply(combinedLayer);
	...
	const recipeOutput = tf.layers
		...
		.apply(combinedLayer) as tf.SymbolicTensor;
...
const prepareTrainingData_IngredientVectors = (ingredients: string[], recipes: JSONObject[]): number[][] => {
	...
	recipes.forEach((recipe: JSONObject, recipeIndex: number) => {
		...
		// 	}
		// });
		const ingredientVector = new Array(ingredients.length).fill(0);
		for (const ingredient of recipe.ingredients) {
            const index = ingredients.indexOf(ingredient);
            if (index !== -1) {
                ingredientVector[index] = 1; // Mark presence
            }
        }
...
const prepareTrainingData_Categories = (categories: string[], recipes: JSONObject[]): JSONObject => {
	...
	recipes.forEach((recipe: JSONObject, recipeIndex: number) => {
        ...
        if (categoryIndex !== -1) {
            categoryVector[categoryIndex] = 1; // Mark presence
        }
		categoryVectors.push(categoryVector);
...
const prepareTrainingData_MealTypes = (mealTypes: string[], recipes: JSONObject[]): JSONObject => {
	...
	recipes.forEach((recipe: JSONObject, recipeIndex: number) => {
        ...
        const mealTypeVector = new Array(mealTypes.length).fill(0);
		...
		// 	}
		// });
		// Fill meal type vector
		for (const mealType of recipe.mealTypes) {
            const index = mealTypes.indexOf(mealType);
            if (index !== -1) {
                mealTypeVector[index] = 1; // Mark presence
            }
        }
		mealTypeVectors.push(mealTypeVector);
...
const prepareTrainingData_DietaryRestrictions = (dietaryRestrictions: string[], recipes: JSONObject[]): JSONObject => {
	...
	recipes.forEach((recipe: JSONObject, recipeIndex: number) => {
		...
		// 	}
		// });
		const dietaryRestrictionVector = new Array(dietaryRestrictions.length).fill(0);
		// Fill dietary restriction vector
		for (const restriction of recipe.dietaryRestrictions) {
			const index = dietaryRestrictions.indexOf(restriction);
			if (index !== -1) {
				dietaryRestrictionVector[index] = 1; // Mark presence
			}
		}
		dietaryRestrictionVectors.push(dietaryRestrictionVector);
